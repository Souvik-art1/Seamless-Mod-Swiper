...
- All files the app creates are stored **in the same folder as the executable**.
...
- **Strict translation exclusion (content-based):**  
  - Exclude entries that appear to be translations **based on description, changelog, and author comments**. Tags are not trusted for this decision.  
  - Prefer content in the selected locale. If text analysis shows a mismatch, exclude it.
    ...
    ```json
    {
      "header": {
        ...
        "filters": {
          ...
          "excludeTranslations": true,   // content-based exclusion
          "locale": "en-US"
        },
        "notes": ""
      },
...
- **Logs, cache, exports:** store everything in the **same folder as the executable**.
...
- **Where cached:** files saved under an internal `cache/` folder next to the executable.  
- **Cache policy:** fixed internal limits for size and freshness. A **Clear Cache** action in Settings deletes the cache folder.  
- **Offline mode:** when enabled, the app does **not** make network requests. It shows only what is already cached. Anything not cached will be unavailable until you go online.
...
- **LRU memory (optional):** keep a small list of the **most recently shown** `(modId, fileId)` pairs.  
  ...
  - The memory lives as a small file next to the executable.
